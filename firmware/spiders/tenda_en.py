...
class TendaENSpider(Spider):
    ...
    url = "http://www.tendacn.com/en/service/download-cata-11-1.html"

    def parse(self, response):
        for page in response.xpath("//*[contains(@class, 'dr_file')]//@href").extract():
            yield Request(
                url=page,
    ...
    def parse_product(self, response):
        url = response.xpath("//*[contains(@class, 'downbtns')]//@href").extract()[0]
        title = response.xpath("//*[contains(@class, 'downdes')]/h1/text()").extract()[0]
        description = title
